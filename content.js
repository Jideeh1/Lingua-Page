(() => {
  ...
  function shouldSkipNode(node) {
    ...
    if (text.length > MAX_NODE_CHARS) return true;
  ...
  function showStatus(message, isError = false) {
    ...
    if (!el) {
      ...
      el.style.cssText =
        "position:fixed;right:12px;bottom:12px;z-index:2147483647;padding:8px 10px;border-radius:8px;background:#111827;color:white;font:12px system-ui;box-shadow:0 4px 14px rgba(0,0,0,.25);max-width:300px;";
  ...
  async function run({ force = false } = {}) {
    ...
    try {
      const hostStatus = await send("IS_HOST_DISABLED", {
        hostname: location.hostname
      });

      if (hostStatus?.error) {
        throw new Error(hostStatus.error);
      }
      ...
      if (!force && settings.autoTranslate === false) {
        return;
      }
      ...
      showStatus(`Translating ${nodes.length} text blocks to ${target}...`);

      for (const node of nodes) {
        await translateNode(node, target);
